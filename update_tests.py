with open('app/src/test/java/com/example/V4AnalyzerTest.kt', 'r') as f:
    content = f.read()
...
import re
content = re.sub(r'    @Test\s+fun testMathematicalBehaviors\(\) \{[\s\S]*?    \}', new_tests, content)

with open('app/src/test/java/com/example/V4AnalyzerTest.kt', 'w') as f:
    f.write(content)
