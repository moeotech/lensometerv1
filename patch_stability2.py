...
replacement = """
            ...
            for ((coord, lensPt) in acceptedMatches) {
                val refPt = gridMap[coord]
                if (refPt != null) {
                    candidateRef.add(refPt)
                    candidateLens.add(lensPt)
                }
            }
            ...
            val q1 = candidateRef.count { it.x > cx && it.y < cy }
"""

orig = """            val candidateRef = mutableListOf<Point>()
            val candidateLens = mutableListOf<Point>()
            for ((coord, lensPt) in acceptedMatches) {
                val refPt = gridMap[coord]
                if (refPt != null) {
