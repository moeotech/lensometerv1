...
# We need to replace from "fun analyzePoints" up to the line before "suspend fun analyze"
start_str = "fun analyzePoints("
end_str = "suspend fun analyze("

start_idx = content.find(start_str)
end_idx = content.find(end_str)

analyze_points_code = """fun analyzePoints(matchedRef: List<Point>, matchedLens: List<Point>, w: Double, h: Double, baseRefDotCount: Int = 0, baseLensDotCount: Int = 0): V4RunResult {
        try {
            val cx = w / 2.0
            val cy = h / 2.0
...
new_content = content[:start_idx] + analyze_points_code + content[end_idx:]
